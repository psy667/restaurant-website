...
class Reserves extends Component {
  ...
  render() {
    const renderItem = (item, i) => (<tr key={item._id}>
      <th>{i + 1}</th>
      <td>{item.name}</td>
      <td><DatePicker className='form-control' style={{
        width: '100%'
      }} selected={new Date(item.day)} onChange={this.handleChangeDate(item._id)} showTimeSelect="showTimeSelect" dateFormat="Pp"/></td>
      ...
      <td>
        <select className="custom-select" style={{
            marginTop: -6
          }} value={item.status} onChange={this.handleChangeStatus(item._id)}>
